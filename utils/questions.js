function getBestFit(coordinates) {
...
function getLineOfBestFit(coordinates) {
  // This function only returns y values -- x values will stay the same
  const dataArrLength = coordinates.length;
  const _a = getBestFit(coordinates), gradient = _a[0], interceptY = _a[1];
...
function isPositiveChange(coordinates) {
